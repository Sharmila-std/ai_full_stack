...
async def evaluate_candidates(prompt: str, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    system_prompt = (
        ...
        "and determine whether their content matches the user's discovery request.\n\n"
        "Rules:\n"
        "1. Focus strictly on whether the profile's content, niche, language, and location match the request.\n"
        "2. Do not match profiles based solely on their username containing the query. You must verify that the bio snippet or description "
        "indicates active content creation or interest in the requested niche.\n"
        "3. If the bio snippet is generic, empty, or does not clearly match the requested niche, set \"match\" to false.\n"
        "4. Provide a realistic match score between 0 and 100 reflecting the relevance of their content (higher score "
        "means stronger content alignment). Do not assign the same score to all candidates.\n"
        "5. Provide a concise, professional reason (1-2 sentences) explaining why they match or do not match.\n"
        "6. Respond ONLY with a valid JSON object matching the schema below. No other text.\n\n"
        "Schema:\n"
        "{\n"
        "  \"evaluations\": [\n"
        "    {\n"
